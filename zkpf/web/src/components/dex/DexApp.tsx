...
const DexAppRoutes = () => {
  ...
  // Move "Chart Trades Data" element to bottom of page
  useEffect(() => {
    const movedElements = new WeakSet<HTMLElement>();
    
    const moveChartTradesData = () => {
      // Find or create container at bottom of page
      let bottomContainer = document.getElementById('chart-trades-data-container');
      if (!bottomContainer) {
        bottomContainer = document.createElement('div');
        bottomContainer.id = 'chart-trades-data-container';
      ...
      const checkAndMove = () => {
        // Find all elements that might contain "Chart Trades Data"
        const allElements = document.querySelectorAll('div');
        allElements.forEach((el) => {
          const text = el.textContent || '';
          if (text.trim() === "Chart Trades Data" || text.includes("Chart Trades Data")) {
            // Check if it has the oui classes matching the pattern
            if (
              el.classList.contains('oui-box') &&
              el.classList.contains('oui-size-width') &&
              el.classList.contains('oui-flex') &&
              el.classList.contains('oui-flex-row') &&
              el.classList.contains('oui-items-center') &&
              el.classList.contains('oui-justify-between') &&
              el.classList.contains('oui-flex-nowrap') &&
              !movedElements.has(el as HTMLElement)
            ) {
      ...
      // Also run after delays to catch dynamically rendered content
      setTimeout(checkAndMove, 1000);
      setTimeout(checkAndMove, 3000);
      setTimeout(checkAndMove, 5000);

      // Use MutationObserver to catch dynamically added content
      const observer = new MutationObserver(checkAndMove);
      observer.observe(document.body, {
        childList: true,
        subtree: true,
      });
      ...
      return () => {
        observer.disconnect();
        // Optionally remove container on cleanup
        // bottomContainer?.remove();
      };
    };

    const cleanup = moveChartTradesData();
    return cleanup;
  }, []);
