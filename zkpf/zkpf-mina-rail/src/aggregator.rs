...
impl ShardAggregator {
    ...
    pub fn finalize_epoch(&mut self, end_slot: u64) -> Result<ShardProof, AggregatorError> {
...
impl EpochAggregator {
    ...
    pub fn finalize_epoch(&mut self, mina_slot: u64) -> Result<EpochProof, AggregatorError> {
        ...
        // Transition to next epoch
        let end_slot = mina_slot;
        self.current_state.end_slot = Some(end_slot);
        self.current_state = self.current_state.next_epoch(end_slot);
