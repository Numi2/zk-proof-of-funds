...
impl StarknetSubmitter {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            private_key,
            client,
            chain_id: chain_id.unwrap_or_else(|| "SN_SEPOLIA".to_string()),
        })
    }
    ...
    fn build_invoke_transaction(
        ...
        let selector = self.cache_attestation_selector();
...
mod tests {
    ...
    #[test]
    fn test_bytes32_to_felts() {
        let bytes = [0u8; 32];
        let (high, low) = bytes32_to_felts(&bytes);
        assert_eq!(high, "0x0");
        assert_eq!(low, "0x0");
    ...
    #[test]
    fn test_encode_attestation_calldata() {
        let submitter = StarknetSubmitter {
            chain_name: "starknet".to_string(),
        ...
        let calldata = submitter.encode_attestation_calldata(&attestation);
        
        // Verify calldata structure
        assert!(!calldata.is_empty());
        // holder_binding (2) + policy_id (1) + epoch (1) + mina_slot (1) + expires_at (1) + state_root (2) + proof_len (1)
