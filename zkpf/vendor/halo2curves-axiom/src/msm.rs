...
// The following types are only used by msm_best which requires multicore
#[cfg(feature = "multicore")]
...
/// Performs a multi-scalar multiplication operation (single-threaded fallback).
///
/// This function will panic if coeffs and bases have a different length.
...
/// This function will panic if coeffs and bases have a different length.
///
/// Single-threaded fallback when multicore is not available.
#[cfg(not(feature = "multicore"))]
pub fn msm_best<C: CurveAffine>(coeffs: &[C::Scalar], bases: &[C]) -> C::Curve {
    // In single-threaded mode, just use the serial implementation
    msm_parallel(coeffs, bases)
}
